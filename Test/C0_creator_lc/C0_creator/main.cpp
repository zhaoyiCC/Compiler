...
#define LOCAL_ARRAY_COUNT 1 // 2
#define LOCAL_VAR_COUNT 1 // 3
...
string get_arrayname()
{
    ...
    return is_global ? get_random_ele(global_arrays) : get_random_ele(local_arrays);
}
...
void print_call_with_expr(string funcname)
{
    OUTPUT(funcname << "(")
    print_expression();
...
void print_item(bool to_use)
{
    typedef void(*handle)();
    RandomGetter<handle> rgetter;
    rgetter.put(print_array, to_use ?       35 : 60 );
    rgetter.put(print_var, to_use ?         35 : 40 );
...
void print_random_call()
{
    if (cur_funcno == FUNC_COUNT - 1)
    {
        return;
    }
    // get func no randomly
    int call_funcno = get_random_num(FUNC_COUNT - 1 - cur_funcno, cur_funcno + 1);
    print_call(funcnames[call_funcno]);
    OUTPUT(";" << endl);
}
...
void print_instructors(int count)
{
    ...
    for (int i = 0; i < count; i++)
    {
        typedef void(*handle)();
        RandomGetter<handle> rgetter;
        rgetter.put(print_assign, 65);
        rgetter.put(print_if, 15);
        rgetter.put(print_printf, 10);
        rgetter.put(print_random_call, 10);
        (*(rgetter.get()))();
    }
    OUTPUT("}" << endl)
}
...
void print_print_all()
{
    stringstream ss;
    ss << output_count++;
    print_print_string("======== " + ss.str() + " : " + funcnames[cur_funcno] + " =========\\n");
    
    print_print_string("glo_vars:");
...
void print_function(int funcno)
{
    if (funcno < 0 || funcno >= FUNC_COUNT)
    {
        return;
    }
    cur_funcno = funcno;
    string funcname = funcnames[funcno];
    OUTPUT("int " << funcname)
