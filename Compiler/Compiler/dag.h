...
int addLeaf(string s){
    ...
    for (int i = cnt_dag; i >= 1; --i){
        if (dag[i].value == s || find(dag[i].names.begin(), dag[i].names.end(), s) != dag[i].names.end()){
            ...
            //不是return i;(代表是第i个dag图的节点是s) 因为我们要保证选择的节点号是最近更新的（存在节点编号从大的叶节点跑到小的非叶节点的情况
        }
    }
    cnt_dag = cnt_dag+1;
...
void addNode(int k){
    ...
    for (auto i: dag[k].names){
        if (i.size() >0 && i[0] == '#'){
            ...
            //!!!需要更正，见下面的if！必须选取和节点表里是一致的临时变量//dag[k].pick = i; //随便选取一个标识符的名字作为这个中间节点的名字代表 //用最后面那个也就是第一次插入的那个，因为names是每次插入到头部即逆向插入的
        }
