...
#include "error.h"
#include "myStd.h"
void consDeclarion(string& str) { // ＜常量说明＞ ::=  const＜常量定义＞;{ const＜常量定义＞;} //＜常量定义＞   ::=   int＜标识符＞＝＜整数＞{,＜标识符＞＝＜整数＞} //char＜标识符＞＝＜字符＞{,＜标识符＞＝＜字符＞}
...
void factor(string& str, string& res){ //＜因子＞    ::= ＜标识符＞｜＜标识符＞‘[’＜表达式＞‘]’｜＜整数＞|＜字符＞｜＜有返回值函数调用语句＞|‘(’＜表达式＞‘)’
    ...
    if (id == 98 || id == 99){ //代表是字符或者(无符号)整数
        ...
        if (sym == "["){
            ...
            if (pos >= 0 && tab[pos].para_num == 0){ //a[1000000000]要报错!!!
                cout << op2+"["+op3+"]" << "!!!ERROR:::Non Array:::" << tab[pos].para_num << "$$$_Line" << mp_line[pos_line_header] << endl;
            }
...
void caseStatement(string& str, string swit_variment, vector<int>& case_v, bool switch_is_char){ //＜情况表＞   ::=  ＜情况子语句＞{＜情况子语句＞}//＜情况子语句＞  ::=  case＜常量＞：＜语句＞
