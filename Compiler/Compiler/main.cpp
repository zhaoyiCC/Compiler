...
//    mcfile.open("asm.txt"); //创建文件 //那个在mipsGenerator的文件是不会创建的，但是之前有asm.txt是写的进去的
    ...
    //    route = "14011100_test.txt";//std::ifstream t(route);//"");//    std::string s((std::istreambuf_iterator<char>(t)),//std::istreambuf_iterator<char>());//transform(s.begin(), s.end(), s.begin(), ::tolower); //全部转成小写，应付while文法的！
    string s = "";
    FILE *fp;
    char c;
    ...
    printQuat(midcode_before_out); //打印又花钱的四元式//*new ofstream("midCodeBefore.txt") //(quat, cnt_quat)
    symbolTable(); //求符号表
    programTable(); //输出程序(函数)信息
    dagWork(); //dag图消除公共表达式
    calcTmp(); //求临时变量等信息
    flowWork(); //计算数据流、构建冲突图、分配全局变量
    quatMips(); //生成mips代码，用mipsGenerator_new头文件则有寄存器分配，用mipsGenr
